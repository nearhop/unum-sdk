...
#define WIRELESS_RADIOS_PATH "/radios"
#define WIRELESS_SCAN_PATH "/neighboring_access_points"
...
static void wireless_do_radio_telemetry(void)
{
    ...
    util_build_url(RESOURCE_PROTO_HTTP, RESOURCE_TYPE_API, url,
    ...
    for(;;) {
        ...
        if(rsp == NULL || (rsp->code / 100) != 2) {
            log("%s: request error, code %d%s %s\n",
                __func__, rsp ? rsp->code : 0, rsp ? "" : "(none)", url);
            break;
        }
